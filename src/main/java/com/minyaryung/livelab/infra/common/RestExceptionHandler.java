...
public class RestExceptionHandler {
    ...
    public ResponseEntity<Map<String, String>> handleHttpClient(HttpClientErrorException ex) {
        ...
        log.warn("LLM request rejected — status={} type={}", status, ex.getClass().getSimpleName());
    ...
    public ResponseEntity<Map<String, String>> handleHttpServer(HttpServerErrorException ex) {
        log.warn("LLM server error — status={} type={}",
    ...
    public ResponseEntity<Map<String, String>> handleNetwork(ResourceAccessException ex) {
        log.warn("LLM network/timeout — type={}", ex.getClass().getSimpleName());
    ...
    public ResponseEntity<Map<String, String>> handleAiNonTransient(NonTransientAiException ex) {
        log.warn("Spring AI non-transient error — type={}", ex.getClass().getSimpleName());
    ...
    public ResponseEntity<Map<String, String>> handleAiTransient(TransientAiException ex) {
        log.warn("Spring AI transient error — type={}", ex.getClass().getSimpleName());
    ...
    public ResponseEntity<Map<String, String>> handleBlogNotFound(BlogPostNotFoundException ex) {
        log.info("blog operation rejected reason=not_found");
        return body(HttpStatus.NOT_FOUND, ex.getMessage());
    }

    @ExceptionHandler(DuplicateBlogSlugException.class)
    public ResponseEntity<Map<String, String>> handleDuplicateBlogSlug(DuplicateBlogSlugException ex) {
        log.info("blog operation rejected reason=duplicate_slug");
    ...
    public ResponseEntity<Map<String, String>> handleValidation(MethodArgumentNotValidException ex) {
        String message = ex.getBindingResult().getFieldErrors().stream()
                .findFirst().map(error -> error.getDefaultMessage()).orElse("입력 값이 올바르지 않습니다.");
        log.info("request validation failed fieldCount={}", ex.getBindingResult().getFieldErrorCount());
        return body(HttpStatus.BAD_REQUEST, message);
    }

    @ExceptionHandler(HttpMessageNotReadableException.class)
    public ResponseEntity<Map<String, String>> handleUnreadable(HttpMessageNotReadableException ex) {
        log.info("request body rejected type={}", ex.getClass().getSimpleName());
        return body(HttpStatus.BAD_REQUEST, "요청 본문 형식 또는 크기가 올바르지 않습니다.");
    }
