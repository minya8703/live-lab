...
public class KafkaMetricsService {
    ...
    public void recordProduced(int n) { runStartedMs = System.currentTimeMillis(); producedTotal.addAndGet(n); }
    ...
    public Snapshot snapshot() {
        long produced = producedTotal.get(), success = successTotal.get(), dlt = dltTotal.get();
        ...
        return new Snapshot(produced, success, dlt, elapsedMs, throughput);
    }

    public void reset() {
        producedTotal.set(0); successTotal.set(0); dltTotal.set(0);
        runStartedMs = 0; lastEventMs = 0;
    }

    public record Snapshot(long produced, long success, long dlt, long elapsedMs, double throughputPerSec) {}
}
