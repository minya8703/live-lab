...
public class DataSeeder {
    ...
    public DataSeeder(ProductRepository repo, @Value("${livelab.demo.product-count}") int targetCount) {
        this.repo = repo;
    ...
    public void seedIfEmpty() {
        ...
        if (existing >= targetCount) {
            log.info("product seed skip — already {} rows (target {})", existing, targetCount);
            return;
        }
        int toInsert = targetCount - (int) existing;
        log.info("product seed start — {} rows (batch {})", toInsert, BATCH_SIZE);
        ...
        for (int i = 0; i < toInsert; i++) {
            ...
            batch.add(p);
            if (batch.size() >= BATCH_SIZE) { repo.saveAll(batch); batch.clear(); }
        }
        if (!batch.isEmpty()) repo.saveAll(batch);
        log.info("product seed done — {} rows / {}ms", toInsert, System.currentTimeMillis() - startMs);
    }
}
