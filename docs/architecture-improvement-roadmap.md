...
| C-08 | P1 | 완료 | 원문 근거 위치 검증 | 원문 각 줄에 번호를 부여하고 모델의 source·line이 시작 시 고정한 문서에 실제 존재하는지 검증한다. |
| C-09 | P1 | 대기 | source-claim 의미 일치 평가 | 검증된 원문 줄이 답변의 각 수치·주장을 실제로 뒷받침하는지 평가한다. |
...
또한 실제 source·line을 붙인 미묘한 허위 주장은 C-09의 source-claim 의미 일치 평가가 필요하다. 공격 문자열을
...
- 시스템 프롬프트에 주입하는 Markdown 각 줄에 `[L번호]`를 부여하고 모델 evidence마다 source와 line을 요구한다.
- 서버는 같은 시작 시 snapshot에서 source·line 범위와 비어 있지 않은 원문 줄을 결정적으로 검사한다.
- 존재하지 않는 source, 범위를 벗어나거나 빈 line, 3개를 넘는 evidence는 모두 fail-closed 보류 응답으로 전환한다.
- 외부 API에는 검증을 통과한 source만 `answer/sources/grounded`로 반환해 내부 원문 전체 노출 범위를 늘리지 않는다.

#### 남은 한계

위치 검증은 원문의 존재와 출처 위조 방지를 보장하지만, 답변의 각 주장과 선택한 원문 줄 사이 의미적 함의를 판정하지는 않는다.
그 평가는 C-09로 분리한다.
