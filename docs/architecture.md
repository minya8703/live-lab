...
경력 데이터 기반 질의응답. 모델 응답을 구조화하고 서버가 source와 원문 line 위치를 검증해 자유 형식의 근거 없는 답변을 그대로 노출하지 않는다.
...
- 모델 출력 계약: `{answer, evidence:[{source, line}], grounded}` JSON
...
- ChatService: `grounded=true`의 source와 `[L번호]` 위치를 검증하고 형식 오류·빈 근거·허위 파일명·범위 밖 또는 빈 line은 고정 보류 응답으로 전환
...
- 남은 한계: source·line의 원문 존재는 검증하지만 선택한 줄과 답변 주장 간 의미 일치는 아직 자동 판정하지 않음
- 재시도: 3회, 1s→2s→8s 백오프
