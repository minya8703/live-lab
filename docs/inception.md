...
| 2026-08-08 | Prometheus·Grafana를 AWS 운영 기본 구성에서 제거 | t4g.small 2GiB에서 모니터링 컨테이너까지 함께 실행했을 때 인스턴스 전반의 응답 저하를 확인. 관측 스택이 핵심 서비스 가용성을 침해한다고 판단해 운영에서는 Spring·Postgres·Redis·Kafka의 자원을 우선하고, Prometheus·Grafana는 로컬 `monitoring` 선택 프로필로 분리. Actuator·Micrometer 계측은 유지하고 인스턴스 상향 또는 외부 관측 환경 확보 시 재도입. |
