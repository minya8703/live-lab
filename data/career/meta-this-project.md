...
- U2: 랜딩 페이지 (완료)
- U3: AI 경력 Q&A 챗봇 — `answer/sources/grounded` 응답 계약과 실제 Markdown source ID 검증
- U4: Redis 캐시 라이브 데모
- U5: Kafka 처리량 데모
...
- 데이터: 이 디렉토리(`data/career/`)의 마크다운 파일 + 시스템 프롬프트 전체 컨텍스트 주입 + 응답 source 존재 검증
...
- **근거 응답은 fail-closed**: JSON 형식 오류, 빈 출처, 존재하지 않는 source ID는 모델 답변을 노출하지 않고 보류 응답으로 전환. 다만 source 내용과 주장 간 의미 일치 검증은 다음 단계.
